...
final class UpdateSession {
    ...
    func startDownload(onFinished: @escaping (_ downloadCompleted: Bool) -> Void) {
...
struct UpdateAvailableView: View {
    ...
    let onClose: (_ downloadCompleted: Bool) -> Void
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            session.startDownload { completed in
                onClose(completed)
            }
        }
    }
...
enum UpdateAvailablePanel {
    ...
    private static var onDismiss: ((Bool) -> Void)?
    ...
    static func show(
        ...
        let hosting = NSHostingController(
            rootView: UpdateAvailableView(session: session) { completed in
                dismiss(notify: true, downloadCompleted: completed)
            }
        )
    ...
    static func dismiss(notify: Bool = true, downloadCompleted: Bool = false) {
        session?.cancelDownload()
        session = nil
        panel?.close()
        panel = nil
        if notify, let onDismiss {
            self.onDismiss = nil
            onDismiss(downloadCompleted)
        } else {
            self.onDismiss = nil
        }
    }
